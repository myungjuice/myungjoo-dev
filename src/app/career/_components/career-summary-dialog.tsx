    ...
    <Dialog>
      <div className='flex items-center justify-between gap-4 rounded-lg bg-slate-100 px-4 py-3 dark:bg-slate-800'>
        <p className='text-body-sm-bold'>
          {t('summary.summaryCount', { count: companies.length })}
        </p>
      ...
      <DialogContent closeLabel={t('summary.close')} className='max-h-[calc(100vh-2rem)]'>
        <DialogHeader>
          <DialogTitle className='text-heading-h5'>{t('summary.title')}</DialogTitle>
          <DialogDescription className='text-body-sm text-slate-600 dark:text-slate-400'>
            {t('summary.count', { count: companies.length })}
          </DialogDescription>
        </DialogHeader>
        <div className='max-h-[70vh] space-y-8 overflow-y-auto px-6 pb-6'>
          {companies.map(company => (
            ...
            >
              <CareerSummaryCompany company={company} />
            </div>
          ))}
        </div>
      </DialogContent>
