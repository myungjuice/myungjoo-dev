...
- PR 초안에는 제목, 본문, 대상 브랜치, Assignee `myungjuice`, 의미 있는 라벨을 포함한다. 사용자 승인 전에는 PR, 푸시, 병합, 태그, GitHub Release를 생성하지 않는다.
- 의존성 변경, 빌드·배포 설정 변경, 사용자 동작·데이터 구조·화면 설계 변경은 구현 전에 승인받는다.
- 구현 완료 전에는 변경 범위에 맞는 테스트와 검증을 실행하고, 결과를 근거와 함께 보고한다.

## 작업 가이드

- 구현 작업 전: `.codex/guides/workflow.md`와 `.codex/guides/engineering.md`
- 브랜치, 이슈, PR, 직접 푸시, 릴리즈 전: `.codex/guides/git-and-release.md`
- CodeRabbit 리뷰 대응과 Codex 코드 리뷰 전: `.codex/guides/review.md`
...
변경 범위에 맞는 최소 검증부터 실행하고, 설정·배포 영향이 있거나 최종 검증이 필요하면 `pnpm build`를 실행한다.
