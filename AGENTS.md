...
- 프로덕션 코드는 구현자와 독립된 코드 리뷰를 받는다. `standard`의 Code Reviewer는 완료
  조건 기반 QA 증거도 확인하고, 실제 사용자 동작·브라우저·넓은 회귀 조건이면 별도 QA를
  추가한다. `high-risk`는 구현자와 분리된 전담 QA와 Code Reviewer를 사용한다.
- 구현 완료 전에는 변경 범위에 맞는 테스트와 검증을 실행하고, 종료 코드와 핵심 결과를
  근거로 보고한다. 최종 전체 검증은 `pnpm verify`로 실행하되, 동일 HEAD의 성공 명령,
  HEAD와 종료 코드가 기록되고 코드·설정·의존성·검증 환경이 그대로면 결과를 재사용한다.
- 새 기능이나 새 GitHub 이슈 구현은 새 Codex 대화에서 시작하고, GitHub issue,
  `AGENTS.md`, 프로젝트 skill·guide와 저장소 상태에서 맥락을 복구하는 것을 기본으로 한다.
