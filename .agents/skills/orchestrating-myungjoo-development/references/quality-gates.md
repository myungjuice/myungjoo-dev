...
다음 표는 역할 라우팅·검증 재사용·대화 운영을 사람이 일관되게 적용하기 위한 참고
계약이다. 구조 검증기는 이 표의 정확한 key·value 조합을 기계 판독하지 않고, 정책 문서에
`승인`, `develop`, `PR`, `외부 변경` 최소 키워드가 있는지만 확인한다. 등급과 증거의 세부
판정은 이 문서와 routing matrix를 함께 읽어 검토한다.
