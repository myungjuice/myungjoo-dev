...
| standard    | 승인된 완료 조건, 테스트 RED/GREEN, 관련 검증, QA, 구현자와 독립된 코드 리뷰 |
...
않는다. `standard`와 `high-risk`의 QA 및 리뷰는 구현자와 분리한다.
...
`pnpm test`, `pnpm build`를 확장한다. `high-risk`는 일부 명령 성공으로 대체하지 않고 전체
`pnpm verify`를 실행한다. hook 실패를 `--no-verify`로 우회하지 않는다.
